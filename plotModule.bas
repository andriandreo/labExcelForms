...
Sub plotCtt()
 ...
 ActiveSheet.ChartObjects("dCttChart").Activate
 M = ActiveChart.SeriesCollection.Count 'Count the total number of Series within the chart
 If V = "Yes" Or V = "Y" Or V = "yes" Or V = "y" Or V = "1" Then
  ActiveChart.ChartTitle.Text = "Time traces for " & n & " consecutive tests at different Vg"
  i = M
 ...
 Do
   ActiveChart.SeriesCollection(i).Select
    ActiveChart.SeriesCollection(i).Values = "='CCalc(" & (M - i + 1) & ")'!dCCalc" & (M - i + 1)
   i = i - 1
 Loop Until i = 0
...
Sub plotCC()
 ...
 Do
   ...
   ActiveChart.SeriesCollection(2 * i).Select
    ActiveChart.SeriesCollection(2 * i).XValues = "='CCalc(" & (M - i + 1) & ")'!$K$5:$K$16"
    ActiveChart.SeriesCollection(2 * i).Values = "='CCalc(" & (M - i + 1) & ")'!$L$5:$L$16"
   i = i - 1
  Loop Until i = 0
  
 'Baseline-substracted Current:
 ActiveSheet.ChartObjects("dCCChart").Activate
 M = ActiveChart.SeriesCollection.Count / 2 'Count the total number of Series within the chart ("=/2" because they are the double)
 If V = "Yes" Or V = "Y" Or V = "yes" Or V = "y" Or V = "1" Then
  i = M
  Do
 ...
 Do
   ActiveChart.SeriesCollection(2 * i - 1).Select
    ActiveChart.SeriesCollection(2 * i - 1).XValues = "='CCalc(" & (M - i + 1) & ")'!xLIN_" & (M - i + 1)
    ActiveChart.SeriesCollection(2 * i - 1).Values = "='CCalc(" & (M - i + 1) & ")'!dyLIN_" & (M - i + 1)
   ActiveChart.SeriesCollection(2 * i).Select
    ActiveChart.SeriesCollection(2 * i).XValues = "='CCalc(" & (M - i + 1) & ")'!$K$5:$K$16"
    ActiveChart.SeriesCollection(2 * i).Values = "='CCalc(" & (M - i + 1) & ")'!$N$5:$N$16"
   i = i - 1
  Loop Until i = 0
...
If V = "Yes" Or V = "Y" Or V = "yes" Or V = "y" Or V = "1" Then
 ActiveSheet.Cells(57, 5) = "Vg (V)"
 ActiveSheet.Cells(57, 6) = "N-Sens (mA/logM/V)"
 ActiveSheet.Cells(57, 7) = "Sens (mA/logM)"
 ActiveSheet.Cells(57, 8) = "Sens (log-1M)"
  ActiveSheet.Cells(57, 8).Characters(Start:=10, Length:=2).Font.Superscript = True
 ActiveSheet.Cells(57, 9) = "R2"
  ActiveSheet.Cells(57, 9).Characters(Start:=2, Length:=1).Font.Superscript = True
 ActiveSheet.Cells(57, 10) = "Lin. Range (logM)"
...
n = Val(Mid(ActiveSheet.Name, InStr(1, ActiveSheet.Name, "(") + 1, 1)) 'Read the number of plot series based on the name of the "CCalc" Sheet
 For i = 1 To n
  ...
  'Write the results within the table
   ...
   ActiveSheet.Cells(66 - i, 9 + j) = "[" & X(1, 1) & ", " & X(M, 1) & "]" 'To write the Linear Range
    ActiveSheet.Cells(66 - i, 9 + j).HorizontalAlignment = xlCenter 'To format (center) the data of the "Lin. Range" column
   ActiveSheet.Cells(66 - i, 8 + j) = rstats(3, 1) 'To write the "R2" coefficient
    ActiveSheet.Cells(66 - i, 8 + j).NumberFormat = "0.0000" 'To format the data in the "R2" column
    ActiveSheet.Cells(66 - i, 8 + j).HorizontalAlignment = xlRight 'To formar (right) the data of the "R2" column
   ActiveSheet.Cells(66 - i, 6 + j) = rstats(1, 1) 'To write the value for the slope (Sensitivity)
  ...
  'Calculate a 2x5 matrix with the linear regression and its stats (Normalised Response):
   ...
   'Write the sensibility for this case:
    ActiveSheet.Cells(66 - i, 7 + j) = rstats(1, 1) 'To write the value for the slope (Normalised Sens.)
     ActiveSheet.Cells(66 - i, 7 + j).NumberFormat = "0.0000E+00" 'To format the data in the "N-Sens" column
     
  'In the case the series correspond to several "Vg":
  If j = 1 Then
    ActiveSheet.Cells(66 - i, 5) = i / 10 'In order to extract the actual "Vg" value based on the "CCalc" Sheet number
    ActiveSheet.Cells(66 - i, 6).Formula = "=F" & (66 - i) & "/D" & (66 - i) 'To write the value for the "Vg"-Normalised  Sensitivity ("N-Sens")
  End If
 Next i
